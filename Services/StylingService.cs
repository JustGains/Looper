...
public static class StylingDefaults
{
    public static List<StylingRule> BuildDefaults() => new()
    {
        ...
        new StylingRule { Name = "compaction banner",
            Pattern = @"^── compaction[^\r\n]*──[─]*$",
            Foreground = "#fbbf24", FontWeight = "Bold" },
        new StylingRule { Name = "error banner",
            Pattern = @"^══[^═\r\n]*error[^═\r\n]*══[═]*$",
            Foreground = "#f87171", FontWeight = "Bold" },
        new StylingRule { Name = "error banner close",
            Pattern = @"^[═]{20,}$",
            Foreground = "#7a3030" },
        new StylingRule { Name = "claude image limit",
            Pattern = @"(?i)image.{0,40}(?:exceed|dimension|2000px)",
            Foreground = "#f87171", FontWeight = "SemiBold" },
        new StylingRule { Name = "claude context limit",
            Pattern = @"(?i)(?:context|token).{0,20}(?:limit|exceed|overflow|too (?:long|large))",
            Foreground = "#f87171", FontWeight = "SemiBold" },
        new StylingRule { Name = "compaction divider",
            Pattern = @"^[─]{20,}$",
            Foreground = "#6e5a1f" },
        new StylingRule { Name = "compact info",
            Pattern = @"^\[compact\][^\r\n]*",
            Foreground = "#fbbf24" },
